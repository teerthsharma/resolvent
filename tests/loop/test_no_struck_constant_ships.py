...
STRUCK: dict[float, str] = {
    -1.389: (
        "M2 decay exponent. WITHDRAWN as a `floor = 1e-6` artifact (MODEL_CARD.md:615, "
        "2026-09-10; the earlier README.md:136 citation moved once README.md's own text "
        "shifted past it -- re-grep for the exponent before trusting either line number). "
    ...
    0.743864: (
        ...
        "PREREGISTERED_RHO_FLOOR -- and scale/negation_scope.py has never defined "
        "any of them: `git log -S\"def <name>(\" --all --oneline -- "
        "scale/negation_scope.py` returns 0 commits for each of the nine, against 1 "
        "for the control `def nrmse(`, which that file does define. For the lead "
        "name the repo-wide form `git log -S\"def absorbing_boundary_kernel(\" --all "
        "--oneline -- \"*.py\" \":!tests/\"` also returns 0, against 3 for the "
        "control `def path_product(`. (The bare `git log -S` across ALL refs is not "
        "an absence proof: it finds this audit's own recordings, and "
        "`def harmonic_measure(` in scale/kirchhoff.py since 22036de, an unrelated "
        "function of the same name. Search corrected 2026-09-11.) This is not a deleted "
