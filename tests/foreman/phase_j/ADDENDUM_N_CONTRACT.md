PHASE J — ADDENDUM N (THE DIAGONAL, AND WHAT TIES THE LEAPS)
...
[Dispatcher note, 2026-09-22 23:56: neither diag_leap.py nor crossties.py exists on this machine (searched Downloads, the repository, Desktop and Documents). Every number in §9 is the author's claim until an instance is rebuilt from this text and re-run.]
...
G_ij only involves tokens j+1..i, so a magnitude zero isolates its segment exactly. The rotation state readable at i is the product since the last absorber: a reset followed by a group, which is one stage of a Krohn–Rhodes cascade. Depth supplies the depth of the cascade (a prediction, R-KR).
...
X3 The bed that separates a hard reset from recency
The A₅ word problem with reset tokens R (corrected generators; §4 C1). The (m,u) head with m = 0 at R and v = 1 only at R is exact. X3a: n = 4096, 212 resets; max error 2.4e−6°, which is the arccos floor.
The opponent is the same SU(2) gate without zeros, attending over the R tokens with a recency slope. X3b: accuracy 0.4429 / 0.8572 / 0.9692 at slopes 0.05 / 0.2 / 1.0.
Honest reading: a steep enough slope approaches exact. Our edge is exactness at every reset density, not accuracy at this one. R-KR must show it with learned weights. The hand-set weights prove representability only.
...
R-KR
Bed: the A₅ word problem with resets, learned. Reset densities {0.02, 0.05, 0.2}, length 4096.
Arms: (f_Q) SU(2) without zeros, (f_QM) the m·u gate, (s_Q) scan only, (a″) FoX, (a‴) PaTH.
Prediction: (f_QM) ≥ 0.99 at every density; (f_Q) falls as density rises; FoX stays at or below the multiset ceiling.
Counter: (f_Q) within 0.01 of (f_QM) at density 0.2, meaning learned recency already does the reset and magnitude buys nothing here.
...
R-KR counter holds → the magnitude leg adds nothing on order beds; X2 stays a theorem without a bed.
...
Foreman: R-SINK P1–P4 on the existing checkpoints first (no training); then R-KR and P5.
...
X3a	reset+group head, max rotation error	2.4e−6°
X3b	recency opponent at slope .05 / .2 / 1	0.4429 / 0.8572 / 0.9692
