"""Chase K1 checkpoint bars, v2. Registered 2026-09-23 ~04:10 before any checkpoint measurement; RED first with
CHASE_STUB=1. Identical to the seven checkpoint bars of test_chase_k1_ckpt.py (their text is below, unchanged) with ONE
change: the instrument is instr2.py, which reads Cameron checkpoints on their own evaluation beds as rdepth.py now builds
them (BED[cfg bed]: S 1024 = module.make_train(default_rng([s_tr, 0])), S 16384 = module.make_test(default_rng([s_te,
16384, 0]), 16384)), so "the S = 16384 bed" in rrange_fR_* is that bed. Names carry the suffix _v2.
The v1 checkpoint bars stay as registered and are not run: Cameron moved R-DEPTH to bed_k' (bed_kp.py) and rdepth.py no
longer exports make_train/make_test, which v1's capture calls. The bf16 and Toeplitz bars are not repeated here.
--- v1 text follows ---
...
ROWS = os.path.join(HERE, "ckpt_rows_v2.jsonl")
results = {}
...
@bar("rrange_fR_depth_R0_v2")
def t_rr_depth():
    _stub()
    return _range_bar("depth")


@bar("rrange_fR_span_R0_v2")
def t_rr_span():
    _stub()
    return _range_bar("span")


@bar("rrange_aL_slope_R0_v2")
def t_rr_al():
    _stub()
...
if __name__ == "__main__":
    ...
    if os.environ.get("CHASE_STUB") == "1":
        ...
        with gpu_lock("chase.k1 ckpt bars v2"):
            for t in tests:
                t()
