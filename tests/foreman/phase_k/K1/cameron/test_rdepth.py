# R-DEPTH at R0 bars (Cameron, it.K1). Written 2026-09-23 before any arm trained. Never edited after its RED.
...
# Eval: n=1024 = 64 held-out train-distribution beds default_rng([13,k]); n in {4096,8192,16384} = the K1.F beds.
#
...
# cameron.k1.rdepth_trained      : every arm/seed passes L-TRAINED.
# cameron.k1.rdepth_learnable    : contract kill. fR held-out n=1024 accuracy (all tokens) >= 0.8, each seed.
# cameron.k1.rdepth_prediction   : fR accuracy beyond depth 2^4 = 16 at n=16384 >= 0.95, each seed.
# cameron.k1.rdepth_multilen16k  : RECORD_K clause 4. fR accuracy (all tokens) at n=16384 >= 0.95, each seed.
# cameron.k1.rdepth_counter_quiet: contract counter does NOT fire. aL4 and aL7 accuracy beyond 2^L <= 0.5 at every
#   test n, each seed. A row > 0.5 = the counter fires (a shortcut the author did not find; the bed is void).
# cameron.k1.rdepth_opponents_quiet: same line for ass4 (beyond 2^4) and aloop5 (beyond 2^5 = 32) at every test n.
# cameron.k1.rdepth_twin_under_line: aL4/aL7 beyond 2^L <= the K1.F (a_L) line (k1f.json, same beds) + 3 SE,
...
def load(arm, s):
    p = HERE / "runs" / f"{arm}_s{s}" / "result.json"
...
for (a, s), r in R.items():
    v = trained(a, s)
    check(f"cameron.k1.rdepth_trained {a} s{s} held-out acc depth<=min(2^{LARM[a]},32) >= 0.9",
...
for s in (0, 1):
    ...
    check(f"cameron.k1.rdepth_learnable fR s{s} held-out n=1024 acc >= 0.8", False if v is None else v >= 0.8,
          "missing" if v is None else f"{v:.4f}")
    r = read("fR", s)
    check(f"cameron.k1.rdepth_prediction fR s{s} beyond16 at n=16384 >= 0.95",
          None if r is None else r["eval"]["16384"]["beyond16"] >= 0.95, "unread/missing" if r is None else r["eval"]["16384"]["beyond16"])
    check(f"cameron.k1.rdepth_multilen16k fR s{s} acc at n=16384 >= 0.95",
          None if r is None else r["eval"]["16384"]["acc"] >= 0.95, "unread/missing" if r is None else r["eval"]["16384"]["acc"])

K1F = json.loads((HERE / "k1f.json").read_text())
for a, L in (("aL4", 4), ("aL7", 7), ("ass4", 4), ("aloop5", 5)):
    for s in (0, 1):
        ...
        for n in TEST_N:
            ...
            name = "cameron.k1.rdepth_counter_quiet" if a in ("aL4", "aL7") else "cameron.k1.rdepth_opponents_quiet"
            ...
            if a in ("aL4", "aL7"):
                ...
                check(f"cameron.k1.rdepth_twin_under_line {a} s{s} n={n} beyond 2^{L} <= line+3SE",
