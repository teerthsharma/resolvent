# Foreman K1 eighth set (k1h reach definition): reach factors of every known window family, and the far-band property. Bars: test_k1h.py.
import json, sys
...
def reach(ok, L):
    """k1h definition: the first band (>= 200 tokens) below 0.5 ends the reach."""
    w = max(1, 2 ** L // 8)
    D = 0
...
def main():
    ...
    (HERE / "k1h.json").write_text(json.dumps(res, indent=1))
    return res
