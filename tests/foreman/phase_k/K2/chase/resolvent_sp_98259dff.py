...
Blocked path (every device and dtype; nothing S x S is ever formed), row blocks of c = 256 rows:
  forward  : per block, logits z (c x hi) against keys <= hi; tau_i by K0 R.sparsemax's sort rule on that block (the whole
             row is present, so tau is exact); W = max(z - tau, 0) (exact zeros); forward substitution
...
def _rows(qs, k, lo, hi, tau=None):
    """Sparsemax weights of rows lo:hi against keys :hi (causal), and their tau (computed unless given)."""
    z = (qs[:, :, lo:hi] @ k[:, :, :hi].transpose(-1, -2)) / math.sqrt(qs.shape[-1])
    z[..., lo:hi].masked_fill_(torch.ones(hi - lo, hi - lo, dtype=torch.bool, device=qs.device).triu(1), float("-inf"))
    if tau is None:
        tau = _tau(z)
    return (z - tau).clamp_(min=0), tau
...
def backward_blocked(qs, k, x, tau, gx, g, c=C):
    ...
    for lo in reversed(range(0, S, c)):
        hi = min(S, lo + c)
        W, _ = _rows(qs, k, lo, hi, tau[:, :, lo:hi])
