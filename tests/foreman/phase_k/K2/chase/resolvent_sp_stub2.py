"""RED stub (stub2 = stub + FRSP) for test_chase_k2.py: the resolvent_sp.py API with a read that ignores the weights, x = (1-g) v,
...
class FRSP:
    """stub2: the FRSP factory over the stub layer (K0 train_ladder ALiBi for layers 0..L-2)."""
    def __new__(cls, d_model, n_heads, layer_idx, n_layers, ctx):
        import importlib.util
        if layer_idx == n_layers - 1:
            return ResolventAttention(d_model, n_heads, layer_idx, n_layers, ctx)
        spec = importlib.util.spec_from_file_location("k0_train_ladder", "C:/Users/seal/Desktop/New folder (32)/tests/foreman/phase_k/K0/wilson/train_ladder.py")
        tl = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(tl)
        return tl.AlibiAttention(d_model, n_heads, layer_idx, n_layers, ctx)
