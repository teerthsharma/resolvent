...
| what blocks it, named | `scale/m3_quintuple.py` has **no `--task` flag** (`m3_capability.py:246` has one) and **saves no per-cell weights** — which is exactly why the fidelity column is empty for every arm. One port, one addition. |
