...
| **the real result** | **Both arms beat softmax on every seed.** `settled − softmax = +0.108437` CI `[+0.068181, +0.147110]` **5/5**; `twin − softmax = +0.111396` CI `[+0.100873, +0.121920]` **5/5**. **The twin's interval is TIGHTER** (`sd 0.016547` vs `0.064106`). **The settling adds variance and no mean.** **PROVENANCE (both intervals on this row).** Estimator is **exact percentile over all `5**5 = 3125` paired resamples**, NOT the `B=10000` Monte-Carlo draw — the two families differ and the difference is not a typo. Computed from `results/m3_quintuple_v2.jsonl`, seeds `0 1 2 3 4`, journal keys `settled_k8_s64_d24_st150_ntr8192_nev512_b21_sd{0..4}`, `twin_k8_s64_d24_st150_ntr8192_nev512_b21_sd{0..4}` and `softmax_k0_s64_d24_st150_ntr8192_nev512_b21_sd{0..4}`; run log `results/m3_quintuple.txt:54`. The Monte-Carlo counterpart of the settled row is `[+0.066232, +0.147110]` and is what `ceq/hf_artifact/README.md` ships, beside this pair and under its own name. Both are recomputed from the journal by `tests/cameron/test_headline_ci_provenance.py`. |
