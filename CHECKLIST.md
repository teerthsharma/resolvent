...
| **the real result** | **Both arms beat softmax on every seed.** `settled − softmax = +0.108437` CI `[+0.068181, +0.147110]` **5/5**; `twin − softmax = +0.111396` CI `[+0.100873, +0.121920]` **5/5**. **The twin's interval is TIGHTER** (`sd 0.016547` vs `0.064106`). **The settling adds variance and no mean.** |
