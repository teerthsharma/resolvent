...
| A4 | **`scale/impact.py` is registered despite its own docstring forbidding it.** `:26` says *"Register as IMPACT in M3_TASKS only past all four gates + planted controls."* It is registered at `:1077` and `:1081`. Its four gates have never executed. | `scale/impact.py:26` | Zero test files import `scale.impact`; zero `results/impact*` artifacts |
...
| B4 | `scale/arm_s.py:55` cites `gram_audit` | `scale/arm_s.py:56-58` | **MOSTLY RETRACTED — Saturn corrected this.** The docstring **already carries the retraction in place** and does not claim the symbol exists. This was not doc rot. What survives is the narrower true claim: **no pivot-coordinate underflow measurement is on record anywhere.** Saturn also caught two line drifts in FINDINGS itself — A4 `:26`→`:23`, A1 `:594`→`:601` |
