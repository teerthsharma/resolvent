...
| iteration | **0 complete (differential), 1 next** |
...
## THE ONE NEXT ACTION (iteration 1)

**Write the M3 kill RED-first, before any arm exists.** Chase: a criterion never
observed failing is a rule, not a test. Build a deliberately broken arm -
predict-the-mean, NRMSE 1.0 by construction - and require every clause of M3's
kill to fire on it. Only then does the windowed arm get its first reading.

Softmax is measured and timestamped FIRST in `results/`, before any signed arm.

## Open REDs

None yet this round. R5 and R8 STRUCK before build (0/20000 event change).
