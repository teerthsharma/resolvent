"""IT.11 MERCURY -- arm_smprime on the same three eval draws as it.10.
...
SEEDS = [2] + [s for s in range(16) if s != 2]
...
out = (ROOT / "results" / "v20_r15_it11_mercury_smprime.jsonl").open("a", encoding="utf-8")
def emit(rec):
    out.write(json.dumps(rec, default=float) + "\n"); out.flush()

emit(dict(t="header", tag="v20_r15_it11_mercury_smprime", task=f"e3_t{T_STAR}",
          t_star=T_STAR, s=S, d=D, d_model=DM, n_train=N_TRAIN, n_eval=N_EVAL,
          steps=STEPS, arms=["arm_smprime"], seeds=SEEDS, eval_seeds=EVAL_SEEDS,
          ...
          why="arm_smprime on 3 eval draws; W1 had 0 draw-checked cells against W3's 18",
