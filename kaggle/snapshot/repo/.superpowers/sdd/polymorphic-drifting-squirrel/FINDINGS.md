...
| C2 | **OVERSTATED THREE WAYS — corrected by Jupiter after fetching the equations.** The original entry read *"softmax is provably Bayes-optimal on exactly that shape."* It is not, on three counts: **(a)** the paper's predictor is **`erf`, not softmax**; **(b)** optimality is **asymptotic under `L = o(d)`**, and this repo runs **`L/d = 4.00` — the opposite direction**; **(c)** the paper's Prop 3 refutes linear **regression**, not linear attention. **What survives:** the *label shape* does match, so **the worry is sound — "provably" is not.** The single-location concern remains a real motivation for the vector-valued corpus; it is not a theorem about this geometry. | `arXiv:2410.01537`, ICLR 2025 — equations fetched under hardened G1. `LOOP_PROMPT.md:34-38` concedes the unqualified version and inherits the same overstatement |
