...
| **`argmax` — the one-hot control** | **WORSE than plain softmax: `−0.118456`, CI `[−0.134115, −0.102786]` — EXACT enumeration over all 3125 paired resamples; the Monte-Carlo family reads `[−0.134115, −0.102204]`.** Collapsing pivot weights to a single reading **loses** what the architecture gains. ~~**The win is not "look up the best pivot" — reading one pivot is worse than reading none. The MIXTURE is the whole contribution and the EQUILIBRIUM is none of it.**~~ **SUPERSEDED R9 — the mixture half is REFUTED, the equilibrium half stands.** `argmax` varies two things at once: it collapses the mixture AND severs the gradient to the gate, since a `scatter_` of zeros and ones is constant in `log_gate`. `argmaxste` (bitwise the same forward, straight-through backward) reads `0.785019`, **`+0.107304` ABOVE softmax at 5/5** and `+0.225760` above `argmax` at 5/5, and against the mixture `argmaxste − twin = −0.004092`, `[−0.029187, +0.023107]`, 2/5. **So `−0.118456` measured untrained selection, not the loss of the mixture. The win IS "look up the best pivot", provided the lookup is trained.** The equilibrium clause is untouched: no `settled` cell is in that comparison. |
...
| the accounting | *"Seven rounds built a routing trick with world-class error bars — that is now proven, not suspected."* ~~`argmax −0.118456` (0/5) ⇒ the **mixture** is the gain;~~ **SUPERSEDED R9: that inference is dead — `argmaxste` isolates the gate gradient and reads `+0.107304` above softmax, so the gain is TRAINED PIVOT SELECTION, not the averaging. See it.9 above.** `settled − twin −0.002959` covering zero ⇒ the fixed point bought nothing — **that half stands**. **But that was the right verdict on tasks with nothing to settle toward.** |
...
| why the near-zero now counts | **First time it is read on a task that PROVABLY requires iteration** — exactly what the previous `−0.002959` could not claim. ~~**And it sharpens against `argmax` rather than repeating it:** the mixture beats a lookup (`argmax −0.118456`),~~ **SUPERSEDED R9 — a TRAINED lookup ties the mixture (`argmaxste − twin −0.004092`, `[−0.029187, +0.023107]`); only an UNTRAINED one loses.** **But a fixed point over the mixture buys nothing measurable** — that clause stands. Two different negatives about two different components. |
