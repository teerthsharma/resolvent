...
**Moved at r10 it.4: live 299 -> 300, orphan 25 -> 24, KEEP 314 -> 315, ATTIC 33 -> 32.** One row,
`scale/vgpe_flops.py`, was re-classed when leg 2 of its P3 conjunction was falsified; see its reason cell.
The 32 ATTIC rows were executed to `attic/` in that iteration and are listed in `attic/ATTIC_MANIFEST.md`.
Rows keep their ORIGINAL path here, because a census is of a commit; the manifest carries the mapping.
...
**`STRUCK.md` at root is a POINTER, not the registry.** R10 P0 it.5 added it to close the P0.4 layout
gap; it is generated by `scripts/render_struck.py` and must never be hand-edited. The registry is
`tests/loop/test_no_struck_constant_ships.py`, in the
...
| `scale/vgpe_flops.py` | Analytic FLOP accounting for the five V12 gauge-positional-encoding arms. | results/arm_s.jsonl (82 rec) | live | KEEP | AMENDED r10 it.4 (was orphan/ATTIC): P3 is a CONJUNCTION -- 0 python importers AND 0 results artifacts -- and LEG 2 IS FALSE. results/vgpe_flops_run.txt is tracked and is this module's own output: `python scale/vgpe_flops.py` at it.4 exits 0 and reproduces that file BYTE-IDENTICALLY, and HOUSE measured 29 of 29 of V12_PRICING.md's four-decimal numbers reproduced from it. A module that reproduces a published table is a live producer, not an orphan. The it.1 cell read "0 results artifacts" off a literal-path scan of results/<name>.jsonl, which is FINDING 1's defect in a second place: this producer writes a .txt, so a census that only looks for a journal cannot see it. NOT MOVED at it.4 |
